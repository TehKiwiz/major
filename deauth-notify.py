...
def myNotification(num):
    if not pynotify.init("icon-summary-body"):
        sys.exit(1)

    n = pynotify.Notification("Deauth flood detected",num+" deauth frames found","notification-message-im")
    n.show()
...
def CaptureTraffic():
    ...
    if IsFileDirExist(tcpdump_cap)=="F":
        ...
        if filesize<300:
            printc ("-", "Use a different wireless interface... please make sure the monitoring interfaceing is working ...","")
    else:
                        ...
                        if FrMAC.find(str(FMAC))!=-1:
                            ...
                            if CLIENTS!=1 and Privacy!="":
                                ...
                                if FMAC!="FF:FF:FF:FF:FF:FF" and FMAC!="Broadcast" and FMAC!="(not associated)" and ESSID!=fcolor.IGray + "<<No ESSID>>":
                                    if PDisplay=="":
                                        printc (" ",fcolor.BWhite + "[ " + fcolor.BBlue + str(FMAC) + fcolor.BWhite + " ]'s SSID Name is--> " + fcolor.BBlue + str(ESSID) ,"")
                                    else:
                                        MACDetail=MACDetail + printc (" ","<$rs$>" + fcolor.BWhite + "[ " + fcolor.BBlue + str(FMAC) + fcolor.BWhite + " ]'s SSID Name is--> " + fcolor.BBlue + str(ESSID) ,"")  + "\n"
...
#                        print "DTYPE2 : " + str(DTYPE2)
        ...
        while listsr<listlen:
            ...
            if int(List_Deauth[listsr])>=10:
                ...
                if int(List_Disassoc[listsr])>=10:
                    ...
                    myNotification(str(List_Deauth[listsr]))
    ...
    if Concern==0:
        if IsFileDirExist(tcpdump_cap)=="F":
            ...
            filesize=statinfo.st_size
            if filesize>=300:
                Result=printc ("-","<$rs$>" + fcolor.SGreen +  "Did not detect any suspicious activity ...\n","")
                #myNotification()
                #os.system("notify-send hackaday.com")
    else:
